...
function InsertRecord(){
	...
	function uploadImg($get_db_user_id) //$get_db_user_id ang sulod ana kai session id sa user
		...
		function insterLoginHistory($db_user_id, $date){
					...
					if($row > 0){
						$updateToLoginHistory = $main_conn->prepare('UPDATE fb_loginhistory set loginStatus = :getStatus AND set loginDate = :getDatenow where userId = :getUserId');
			...
			while($row = $OnlineUsersQuery->fetch(PDO::FETCH_ASSOC))
			{
				...
				$db_fullname = ucwords($row['firstname']) ." ". ucfirst($row['lastname']);
						...
						if($photo_user_id != ''){
							...
							$photo_user_id ="../img/fbcover.jpg' class='img_thumbnail_cover'";
							$Online_Users[] = array(
								'user_id' => $row['ownId'],
								'name' => $db_fullname,
								'image' => $photo_user_id,
